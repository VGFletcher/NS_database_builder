#This code calculates the Helmholtz free energy of BCC and FCC in a pymatnest nested sampling run for
...
for i in range(t_per_thread):
    ...
    fcc_lz = -Kb*T*(np.log(np.sum(Z[fcc])) + shift)
    bcc_lz = -Kb*T*(np.log(np.sum(Z[bcc])) + shift)
    
    np.savetxt(res_file, np.reshape([T, fcc_lz, bcc_lz], (1,3)))
res_file.close()
comm.barrier()
